...
namespace ExampleUserDefineData
{
    ...
    {
        List<ExampleUserDefineData.DAL.UserDefData> CorrectableRecs = new List<ExampleUserDefineData.DAL.UserDefData>();
        ...
        public DataRationalityMessage Execute()
        {
            ...
            // 取得學生自訂欄位資料
            List<ExampleUserDefineData.DAL.UserDefData> UserDefDataList = UDTTransfer.GetDataFromUDT(studIDDict.Keys.ToList());

            // 檢查資料並組合
            Dictionary<string, Dictionary<string, List<ExampleUserDefineData.DAL.UserDefData>>> dataDict = new Dictionary<string, Dictionary<string, List<DAL.UserDefData>>>();

            foreach (ExampleUserDefineData.DAL.UserDefData data in UserDefDataList)
            {
            ...
            try
            {
                ...
                {
                    Dictionary<string, List<ExampleUserDefineData.DAL.UserDefData>> dataA = dataDict[sid];
                    foreach (List<ExampleUserDefineData.DAL.UserDefData> data in dataA.Values)
                    {                        
                        // 有重複
                        if (data.Count > 1)
                        {
                            foreach (ExampleUserDefineData.DAL.UserDefData data1 in data)
                            {
